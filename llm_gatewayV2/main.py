...
async def chat(req: ChatRequest):
    ...
    for _ in range(len(candidates) + 1):
        ...
        try:
            ...
            upstream_5xx = status_code is not None and 500 <= status_code < 600
            # Always failover on 5xx upstream errors regardless of explicit_override —
            # those are infrastructure failures, not config/auth issues.
            if (explicit_override and not upstream_5xx) or not getattr(e, "retryable", True):
