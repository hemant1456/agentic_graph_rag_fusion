...
step_02_chunking/
  implementation/
    ...
    pipeline.py        Step02RAG — same .build()/.query() interface
  evaluation/
...
```bash
uv run pytest step_02_chunking/tests/ -v
```
...
from step_02_chunking.implementation.chunker import load_and_chunk
corpus = Path('dataset/company_data')
...
```bash
uv run python step_02_chunking/implementation/ingestor.py
```
