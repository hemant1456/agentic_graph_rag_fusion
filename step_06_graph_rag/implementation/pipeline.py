"""
Step 06 RAG pipeline — Graph RAG with alias resolution and aggregate queries.

Upgrades over Step 05:
  - Entity resolution: exact name match + alias/keyword match
  - Aggregate graph queries (e.g. enterprise ARR %) at query time
  - Dependency chain tracing (full blast-radius expansion)

Reuses: Step 04 ChromaDB vector index, Step 05 knowledge graph.
"""
...
class Step06RAG:
    """
    Graph RAG pipeline: vector retrieval + alias-resolved graph context
    + on-the-fly aggregate queries over the knowledge graph.

    Usage:
        rag = Step06RAG(k=10).build()
        result = rag.query("What % of ARR comes from enterprise customers?")
    """
