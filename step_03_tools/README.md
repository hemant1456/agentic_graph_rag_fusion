...
## Design
- **Class:** `Step03ToolsRAG` in `step_03_tools/implementation/pipeline.py`
- **Inherits from:** composes `Step02RAG`'s ChromaDB collection and retrieval; reuses `BaselineRAG`'s generation
- **Key components:**
  - `step_03_tools/implementation/csv_tool.py` — `detect_intent()` regex/keyword router and `run_query()` pandas executor
...
## Run
```bash
uv run python evaluation/run_eval.py --step step_03_tools
```

## Results
See `step_03_tools/results/eval_results.json` for the latest RAGAS scores.
