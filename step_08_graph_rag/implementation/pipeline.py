...
from step_07_knowledge_graph.implementation.pipeline import Step07RAG

CORPUS_PATH = Path(__file__).parent.parent.parent / "step_00_dataset" / "company_data"
GRAPH_PATH  = Path(__file__).parent.parent.parent / "step_07_knowledge_graph" / "results" / "graph.json"


class Step08RAG(Step07RAG):
    """
    Extends Step07RAG with alias-resolved graph context and BFS dependency expansion.

    Usage:
        rag = Step08RAG(k=10).build()
