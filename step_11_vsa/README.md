...
1. Create `step_11_vsa/implementation/slices/myslice.py` with `CONFIG: SliceConfig` and `can_handle(question) -> float`
...
| `implementation/pipeline.py` | `Step11RAG` with `query_extended()` → `Step11Result` |
