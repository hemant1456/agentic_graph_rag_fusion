...
- **Inherits from:** composes `Step03HybridRAG` (k=20 wide candidates) and reuses the Step 07 agents
- **Key components:**
  - `step_06_context_engineering/implementation/reranker.py` — CrossEncoder rerank to top `rerank_k=8`
  - `step_06_context_engineering/implementation/deduplicator.py` — Jaccard near-duplicate removal
  - `step_06_context_engineering/implementation/compressor.py` — sentence-level extractive compression to `compress_ratio=0.60`
