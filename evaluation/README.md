...
uv run python evaluation/evaluation/run_eval.py

# Aggregate scores + per-question table
uv run python evaluation/implementation/report.py --summary

# Full detail for one question
uv run python evaluation/implementation/report.py --question Q01

# All questions ranked by worst context recall
uv run python evaluation/implementation/report.py --worst context_recall

# Unit tests (no API keys needed)
uv run pytest evaluation/tests/ -v -k "not integration"

# Integration test (requires API keys + built index)
uv run pytest evaluation/tests/ -v -m integration
```
