...
## Design
- **Class:** `Step08RAG` in `step_08_production/implementation/pipeline.py`
- **Inherits from:** composes `Step07RAG`
- **Key components:**
  - `step_08_production/implementation/semantic_cache.py` — `SemanticCache` with a configurable cosine threshold (default `0.92`)
  - `step_08_production/implementation/retry.py` — `@with_retry(max_attempts=3, base_delay=0.5)` decorator
  - `step_08_production/implementation/graceful_degradation.py` — `extractive_fallback()` returns a chunk-derived answer when retries fail
  - `step_08_production/implementation/confidence.py` — `score_answer()` produces a numeric score plus high/medium/low label
  - `step_08_production/implementation/health_monitor.py` — `HealthMonitor` with a 100-query rolling window for latency, pass rate, cache hits, and slice distribution
...
## Results
See `step_08_production/results/eval_results.json` for the latest RAGAS scores.
